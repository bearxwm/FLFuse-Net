...
__all__ = ["Compose", "ToTensor", "ToPILImage", "Normalize", "Resize", "Scale", "CenterCrop", "Pad",
           ...
           "RandomPerspective", "RandomErasing", "MyRandomCrop"]
...
class MyRandomCrop(object):
    ...
    @staticmethod
    def get_params(img_a, output_size):
        """Get parameters for ``crop`` for a random crop.
    ...
    def __call__(self, img_a, img_b):
        ...
        """
        if self.padding is not None:
            img_a = F.pad(img_a, self.padding, self.fill, self.padding_mode)
            img_b = F.pad(img_b, self.padding, self.fill, self.padding_mode)

        # pad the width if needed
        if self.pad_if_needed and img_a.size[0] < self.size[1]:
            img_a = F.pad(img_a, (self.size[1] - img_a.size[0], 0), self.fill, self.padding_mode)
            img_b = F.pad(img_b, (self.size[1] - img_a.size[0], 0), self.fill, self.padding_mode)
        # pad the height if needed
        if self.pad_if_needed and img_a.size[1] < self.size[0]:
            img_a = F.pad(img_a, (0, self.size[0] - img_a.size[1]), self.fill, self.padding_mode)
            img_b = F.pad(img_b, (0, self.size[0] - img_a.size[1]), self.fill, self.padding_mode)

        i, j, h, w = self.get_params(img_a, self.size)


        return F.crop(img_a, i, j, h, w), F.crop(img_b, i, j, h, w)
